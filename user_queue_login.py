        ...
        with db_connector.cursor() as mycursor:
            ...
            for x in mycursor.fetchall():
                ...
                if cookie_setup:
                    ...
                    if not exist_info:
                        mycursor.execute(f"INSERT INTO mx_account_info (queue_id,uid,account_id,username,avatar,extend,type,status) VALUES ({queue_id},{account_id},'{cookie_setup['account_id']}','{cookie_setup['username']}','{cookie_setup['avatar']}','',{type},1)")
                    else:
                        # 存在则判断是否登录，未登录则改为登录
                        if exist_info[1] !=1:
